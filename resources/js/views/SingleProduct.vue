        ...
        <div class="row">
            <div class="col-md-8 offset-md-2">
                <div v-for="(product, index) in product" :key="index">
                    ...
                    <h4>
                        <span class="small-text text-muted float-left">{{formatCurrency(product.price)}}</span>
            ...
            <Dialog header="Write a review" :visible.sync="display" :style="{width: '50vw'}" >
                <!-- <div class="p-field">
                    <img :src="'/images/'+product[0].image" :alt="product.name" height="50px" width="50px">
                </div> -->
                <div class="p-field">
                    <label for="rating">Rating</label>
                    <Rating id="rating" v-model="review.rating" class="p-mr-3" :cancel="false"></Rating>
                </div>                
                <div class="p-field">
                    <label for="title">Title</label><br/>
                    <InputText id="title" style="with:100%!important;" v-model="review.title" required="true"/>
                </div>
                <div class="p-field">
                    <label for="description">Description</label><br/>
                    <TextArea id="description" v-model="review.description" required="true" rows="3" cols="20"/>
                </div>
                <!--<div class="p-field">
    ...
    export default {
        ...
        methods : {
            ...
            addReview() {
                ...
                let product_rating = this.product.product_rating
            ...
            getReviews() {
                this.user = this.user = JSON.parse(localStorage.getItem('vue-laravel-ecommerce.user'))
                ...
                axios.get(url).then(response => {
                    ...
                    let findReview = product_rating.find(o => o.id === this.user.id)
                    if ( findReview ) {
                        this.hasReview = true
                    }
